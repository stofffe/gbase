@group(0) @binding(0) var<uniform> settings: WeatherMapUniforms;
struct WeatherMapUniforms {
    size: u32,
}
@group(0) @binding(1) var output: texture_storage_2d<rgba8unorm, write>;

const POINT_OFFSET_MULT = 0.6;
const CUTOFF = 0.4;
const CELLS = 5;
...
fn cs_main(@builtin(global_invocation_id) id: vec3<u32>) {
    var density = worley_2d(id.xy, settings.size, u32(CELLS));
    // density = saturate(remap(density, 0.1, 1.0, 0.0, 1.0));
    // density = smoothstep(0.5, 0.6, density);
    // density = step(0.5, density);
    // density = remap(density, 0.0, 0.5, 0.0, 1.0);

    let color = vec4f(density, density, density, 1.0);
    textureStore(output, id.xy, color);
}

// generate worley
fn worley_2d(coord: vec2<u32>, size: u32, cells: u32) -> f32 {
    let cell_size = i32(size / cells);
    let center_cell = vec2<i32>(coord) / cell_size;
    let pixel_pos = vec2<i32>(coord);

    var closest_dist = f32(size);
    var second_dist = closest_dist;
    var closest_hash = 0u;

    for (var i = -1; i <= 1; i++) {
        for (var j = -1; j <= 1; j++) {
            let cell_index = center_cell + vec2<i32>(i, j);

            let cell_index_wrapped = mod_vec2_i32(cell_index, i32(cells));
            let cell_hash = hash_2d(u32(cell_index_wrapped.x), u32(cell_index_wrapped.y));

            let cell_pos = vec2<f32>(cell_index * cell_size) + f32(cell_size / 2);
            let cell_pos_jittered = cell_pos + hash_to_vec2_snorm(cell_hash) * f32(cell_size) * POINT_OFFSET_MULT;

            let dist = length(vec2<f32>(pixel_pos) - cell_pos_jittered);
    ...
    let close = closest_dist / f32(cell_size);
    let second = second_dist / f32(cell_size);
    let diff = abs(close - second);
    // if diff < CUTOFF {
    //     return 0.0;
    // }
    // let diff = (second_dist - closest_dist) / f32(cell_size);
    // if (second_dist - closest_dist) / f32(cell_size) < 0.2 {
    //     return 0.0;
    // }
    // var dist = closest_dist / f32(cell_size);
    // dist += (diff);

    // return 1.0 - dist / (closest_dist / second_dist);
    // return 1.0 - second_dist / f32(cell_size);
    return close;
}
